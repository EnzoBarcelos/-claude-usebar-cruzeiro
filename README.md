...
vivo** até a próxima atualização do uso. O popup é **responsivo** (fontes, barras e espaçamentos
escalam ao redimensionar), pode ser **fixado** (alfinete no canto superior direito) e lembra
posição/tamanho entre execuções. Opcionalmente, exibe uma **imagem de fundo** (modo cover) sob
um véu escuro de intensidade configurável.

<p align="center">
  <img src="https://raw.githubusercontent.com/EnzoBarcelos/-claude-usebar-cruzeiro/main/docs/Cruzeiro.png"
       alt="Popup do claude-usebar aberto a partir do ícone na bandeja"
       width="420">
</p>
...
- **Clique no ícone da bandeja** abre/fecha o popup.
- **Arrastar a faixa superior** move o popup pela tela; a posição é salva.
- **Arrastar as bordas/cantos** redimensiona; fontes e barras escalam proporcionalmente, e o
  tamanho é salvo.
- **Alfinete (canto superior direito)** fixa o popup — quando fixado, não some ao perder o
  foco e reabre fixado no próximo logon.
...
- **Ícone: 5h/7d/max** — alterna qual janela o `%` do ícone reflete.
- **Cor** — escolhe entre 5 temas (Vermelho, Roxo, Azul, Verde, Laranja); cada tema define
  um gradiente de cores por severidade (`low`/`mid`/`high`/`critical`).
- **Som ao atualizar** — liga/desliga o som do "Atualizar agora".
...
```jsonc
{
  ...
  "mode": "5h",                // qual % vai no ícone: "5h" (sessão) | "7d" | "max"
  "showRemaining": true,
  "iconStyle": "pct",
  "colorTheme": "vermelho",    // "vermelho" | "roxo" | "azul" | "verde" | "laranja"
  "notifications": false,      // balões de aviso em estados de erro
  "soundEnabled": true,        // som ao clicar "Atualizar agora"
  "pinned": false,             // popup fixado (não some ao perder o foco)
  "popupWidth": null,          // largura lembrada do popup (null = automático)
  "popupHeight": null,         // altura lembrada do popup (null = automático)
  "popupX": null,              // posição X (null = canto inferior direito da tela)
  "popupY": null,              // posição Y
  "backgroundImage": null,     // caminho da imagem de fundo (null = fundo sólido)
  "backgroundDarken": 0.75,    // véu escuro sobre a imagem: 0 (sem véu) a 0.95 (quase preto)
  "colors": {                  // resolvido a partir de colorTheme; edite colorTheme, não isso
    "low": "#7f1010", "mid": "#b71c1c", "high": "#e53935", "critical": "#ff1744"
  }
}
```
...
- **Modelo em uso:** lê o transcript `.jsonl` mais recente em `%USERPROFILE%\.claude\projects`
  e extrai o último `model` (ex.: `claude-opus-4-7` → `Opus 4.7`), recheckado a cada 15 s.
...
- **Instância única:** mutex global `Global\claude-usebar` impede duas instâncias simultâneas.
